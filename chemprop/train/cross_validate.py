        ...
        if args.dataset_type == 'classification':
            if test_target.shape[1]==1:
                xgb_gbc = xgb.XGBClassifier(base_score=0.5, booster='gbtree', colsample_bylevel=1,
                                            ...
                                            max_depth=6,eta=0.2,missing=None, n_estimators=2000,
                ...
                # Sp = tn/(tn+fp)
                # acc = accuracy_score(test_target, pre_pro)
                dmpnn_morgan_scores.append([AUC])
            ...
            if test_target.shape[1]==1:
                ...
                MSE = mean_squared_error(y_test, y_pred)
                RMSE = MSE ** 0.5
                dmpnn_morgan_scores.append([RMSE])
                joblib.dump(xgb_gbc, 'external_test/dmpnn_morgan_xgb.model')
...
def read_model_xgboost(args: TrainArgs, logger: Logger = None) -> Tuple[float, float]:
    """k-fold cross validation"""
    info = logger.info if logger is not None else print
    ...
    for fold_num in range(args.num_folds):
        ...
        makedirs(args.save_dir)
        model_scores = 0.7
        model = load_checkpoint('btk_c/depth_3_dropout_0.15000000000000002_ffn_num_layers_3_hidden_size_2300/fold_0/model_0/model_epoch_last.pt', device=args.device, logger=logger)
        dmpnn_scores.append(model_scores)
        train_target, train_feature, val_target, val_feature, test_target, test_feature,train_smiles,val_smiles,test_smiles = get_xgboost_feature(args, logger,model)
        train_target = pd.DataFrame(train_target)
        train_feature = pd.DataFrame(train_feature)
...
def vs_predict(args,model,logger,external_test_path):
    # model = load_checkpoint(path, device=args.device, logger=logger)
    ...
    input_data.to_csv('external_test/extra_vs_24_3_predict.csv',index=None)

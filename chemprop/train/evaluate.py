...
def judge(model: nn.Module,
    ...
    for batch in tqdm(data_loader, total=len(data_loader)):
        ...
        for i in preds.cpu().detach().numpy():
            preds_prob.append(i[0])
    numbers = 0
    for i in preds_prob:
        if i>0.9:
            numbers = numbers+1
    # import pandas as pd
    # df = pd.DataFrame({'preds_prob': preds_prob, 'val_target': [int(i[0]) for i in targets]})
    # df = df.sort_values(by="preds_prob", ascending=False)
    # df.index = range(df.shape[0])
    # a = int(df[df.val_target==1].index.tolist()[0])
    return numbers

...
def run_training(args: TrainArgs, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        test_preds, test_feature = predict(
            model=model,
    ...
    return ensemble_scores, model, scaler

def get_xgboost_feature(args, logger,model):
    if logger is not None:
        debug, info = logger.debug, logger.info
    else:
        debug = info = print
    ...
    train_preds, train_feature = predict(
        model=model,
        data_loader=train_data_loader,
        scaler=scaler
    )
    val_preds, val_feature = predict(
        model=model,
        data_loader=val_data_loader,
        scaler=scaler
    )
    ...
    return train_targets, train_feature,val_targets, val_feature,test_targets, test_feature,train_smiles,val_smiles,test_smiles
...
def predict_feature(args,logger,model,external_test_path):
    ...
    a = []
    for i in external_test_preds:
        for j in i:
            a.append(j)
    
    external_test_preds = a       
